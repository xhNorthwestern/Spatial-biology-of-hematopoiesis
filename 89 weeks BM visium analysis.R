results_folder = '~/Desktop/Visium/The fifth four adult mouse samples/89weeks_BM/2'
...
data_path = '~/Desktop/Visium/The fifth four adult mouse samples/89weeks_BM'

visium_89WBM = createGiottoVisiumObject(visium_dir = data_path,
                                        expr_data = 'raw',
                                        png_name = 'tissue_lowres_image.png',
                                        gene_column_index = 2,
                                        instructions = instrs)

showGiottoImageNames(visium_89WBM)

pDataDT(visium_89WBM)

spatPlot2D(gobject = visium_89WBM, cell_color = 'in_tissue', point_size = 2,
           cell_color_code = c('0' = 'lightgrey', '1' = 'blue'),
           show_image = T, image_name = 'image')

metadata = pDataDT(visium_89WBM)
in_tissue_barcodes = metadata[in_tissue == 1]$cell_ID
visium_89WBM = subsetGiotto(visium_89WBM, cell_ids = in_tissue_barcodes)


visium_89WBM <- filterGiotto(gobject = visium_89WBM,
...
visium_89WBM <- normalizeGiotto(gobject = visium_89WBM, scalefactor = 6000, verbose = T)


visium_89WBM <- addStatistics(gobject = visium_89WBM)


spatPlot2D(gobject = visium_89WBM, show_image = T, point_alpha = 0.7,
           cell_color = 'nr_feats', color_as_factor = F)


visium_89WBM <- calculateHVF(gobject = visium_89WBM, save_plot = TRUE)

gene_metadata = fDataDT(visium_89WBM)
featgenes = gene_metadata[hvf == 'yes' & perc_cells > 3 & mean_expr_det > 0.4]$feat_ID


visium_89WBM <- runPCA(gobject = visium_89WBM,
                       feats_to_use = featgenes)

screePlot(visium_89WBM, ncp = 30)

plotPCA(gobject = visium_89WBM)


visium_89WBM <- runUMAP(visium_89WBM, dimensions_to_use = 1:10)
plotUMAP(gobject = visium_89WBM)

visium_89WBM <- runtSNE(visium_89WBM, dimensions_to_use = 1:10)
plotTSNE(gobject = visium_89WBM)


visium_89WBM <- createNearestNetwork(gobject = visium_89WBM, dimensions_to_use = 1:10, k = 15)


visium_89WBM <- doLeidenCluster(gobject = visium_89WBM, resolution = 0.6, n_iterations = 1000)


dimPlot2D(gobject = visium_89WBM,     dim_reduction_name = 'umap',
...
dimPlot2D(gobject = visium_89WBM,     dim_reduction_name = 'umap',
          cell_color = "leiden_clus", show_NN_network = F, label_size = 10, show_center_label = F, point_size = 7.0, show_legend = F, legend_text = 20,
          legend_symbol_size = 6, axis_text = 16, axis_title = 16,
          save_param = list(save_name = "7_c_Annotation"))


spatDimPlot(gobject = visium_89WBM, cell_color = 'leiden_clus',
            dim_point_size = 2, spat_point_size = 2.5)

spatDimPlot(gobject = visium_89WBM, cell_color = 'nr_feats', color_as_factor = F,
            dim_point_size = 2, spat_point_size = 2.5)


spatPlot2D(visium_89WBM, cell_color = 'leiden_clus', label_size = 10,
           coord_fix_ratio = 1, point_size = 4.0, show_legend = F, axis_text = 16,
           axis_title = 16)

spatPlot2D(visium_89WBM, cell_color = 'leiden_clus',
           group_by = 'leiden_clus', coord_fix_ratio = 1,
           cow_n_col = 4, show_legend = F,
           save_param = list(base_width = 14, base_height = 14))


gini_markers_subclusters = findMarkers_one_vs_all(gobject = visium_89WBM,
                                                  method = 'gini',
...
violinPlot(visium_89WBM, feats = unique(topgenes_gini), cluster_column = 'leiden_clus',
           strip_text = 8, strip_position = 'right',
           save_param = list(base_width = 5, base_height = 10))


plotMetaDataHeatmap(visium_89WBM, selected_feats = unique(topgenes_gini),
                    metadata_cols = c('leiden_clus'),
                    x_text_size = 10, y_text_size = 10)


dimFeatPlot2D(visium_89WBM, expression_values = 'scaled',
...
scran_markers_subclusters = findMarkers_one_vs_all(gobject = visium_89WBM,
                                                   method = 'scran',
                                                   expression_values = 'normalized',
                                                   cluster_column = 'leiden_clus')
...
violinPlot(visium_89WBM, feats = unique(topgenes_scran), cluster_column = 'leiden_clus',
           strip_text = 10, strip_position = 'right',
           save_param = list(base_width = 5))


plotMetaDataHeatmap(visium_89WBM,  x_text_size = 20,
                    x_text_angle = 45,
                    y_text_size = 20,
...
visium_89WBM = runPAGEEnrich(gobject = visium_89WBM, sign_matrix = PAGE_matrix_1)

cell_types_PAGE = colnames(PAGE_matrix_1)
plotMetaDataCellsHeatmap(gobject = visium_89WBM,
...
spatCellPlot2D(gobject = visium_89WBM,
               spat_enr_names = 'PAGE',
               cell_annotation_values = cell_types_PAGE[13:16],
               cow_n_col = 2,coord_fix_ratio = 1, point_size = 3.0, show_legend = T)
...
write_xlsx(Spatialcorrelation@enrichDT, "~/Desktop/Visium/The fifth four adult mouse samples/89weeks_BM/2/89weeks BM correlation analysis.xlsx")
