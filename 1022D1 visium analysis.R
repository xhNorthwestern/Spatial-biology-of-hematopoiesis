results_folder = '~/Desktop/Visium/The fourth sixteen human bone marrow samples/NMB22-1022_D1'
...
data_path = '~/Desktop/Visium/The fourth sixteen human bone marrow samples/NMB22-1022_D1'

visium_1022D1 = createGiottoVisiumObject(visium_dir = data_path,
                                         expr_data = 'raw',
                                         png_name = 'tissue_lowres_image.png',
                                         gene_column_index = 2,
                                         instructions = instrs)

showGiottoImageNames(visium_1022D1)

pDataDT(visium_1022D1)

spatPlot2D(gobject = visium_1022D1, cell_color = 'in_tissue', point_size = 2,
           cell_color_code = c('0' = 'lightgrey', '1' = 'blue'),
           show_image = T, image_name = 'image')

metadata = pDataDT(visium_1022D1)
in_tissue_barcodes = metadata[in_tissue == 1]$cell_ID
visium_1022D1 = subsetGiotto(visium_1022D1, cell_ids = in_tissue_barcodes)


visium_1022D1 <- filterGiotto(gobject = visium_1022D1,
...
visium_1022D1 <- normalizeGiotto(gobject = visium_1022D1, scalefactor = 6000, verbose = T)


visium_1022D1 <- addStatistics(gobject = visium_1022D1)


spatPlot2D(gobject = visium_1022D1, show_image = T, point_alpha = 0.7,
           cell_color = 'nr_feats', color_as_factor = F)


visium_1022D1 <- calculateHVF(gobject = visium_1022D1, save_plot = TRUE)

gene_metadata = fDataDT(visium_1022D1)
featgenes = gene_metadata[hvf == 'yes' & perc_cells > 3 & mean_expr_det > 0.4]$feat_ID


visium_1022D1 <- runPCA(gobject = visium_1022D1,
                        feats_to_use = featgenes)

screePlot(visium_1022D1, ncp = 30)

plotPCA(gobject = visium_1022D1)


visium_1022D1 <- runUMAP(visium_1022D1, dimensions_to_use = 1:10)
plotUMAP(gobject = visium_1022D1)


visium_1022D1 <- createNearestNetwork(gobject = visium_1022D1, dimensions_to_use = 1:10, k = 15)


visium_1022D1 <- doLeidenCluster(gobject = visium_1022D1, resolution = 0.4, n_iterations = 1000)

dimPlot2D(gobject = visium_1022D1,     dim_reduction_name = 'umap',
...
spatDimPlot(gobject = visium_1022D1, cell_color = 'leiden_clus',
            dim_point_size = 2, spat_point_size = 2.5)

spatDimPlot(gobject = visium_1022D1, cell_color = 'nr_feats', color_as_factor = F,
            dim_point_size = 2, spat_point_size = 2.5)


spatPlot2D(visium_1022D1, cell_color = 'leiden_clus', label_size = 10,
           coord_fix_ratio = 1, point_size = 3.5, show_legend = F, axis_text = 16,
           axis_title = 16)

spatPlot2D(visium_1022D1, cell_color = 'leiden_clus',
           group_by = 'leiden_clus', coord_fix_ratio = 1,
           cow_n_col = 4, show_legend = F,
           save_param = list(base_width = 14, base_height = 14))


gini_markers_subclusters = findMarkers_one_vs_all(gobject = visium_1022D1,
                                                  method = 'gini',
...
violinPlot(visium_1022D1, feats = unique(topgenes_gini), cluster_column = 'leiden_clus',
           strip_text = 8, strip_position = 'right',
           save_param = list(base_width = 5, base_height = 10))


plotMetaDataHeatmap(visium_1022D1, selected_feats = unique(topgenes_gini),
                    metadata_cols = c('leiden_clus'),
                    x_text_size = 10, y_text_size = 10)


dimFeatPlot2D(visium_1022D1, expression_values = 'scaled',
...
scran_markers_subclusters = findMarkers_one_vs_all(gobject = visium_1022D1,
                                                   method = 'scran',
                                                   expression_values = 'normalized',
                                                   cluster_column = 'leiden_clus')
...
violinPlot(visium_1022D1, feats = unique(topgenes_scran), cluster_column = 'leiden_clus',
           strip_text = 10, strip_position = 'right',
           save_param = list(base_width = 5))


plotMetaDataHeatmap(visium_1022D1,  x_text_size = 20,
                    x_text_angle = 45,
                    y_text_size = 20,
...
visium_1022D1 = runPAGEEnrich(gobject = visium_1022D1, sign_matrix = PAGE_matrix_1)

cell_types_PAGE = colnames(PAGE_matrix_1)
plotMetaDataCellsHeatmap(gobject = visium_1022D1,
...
write_xlsx(Spatialcorrelation@enrichDT, "~/Desktop/Visium/The fourth sixteen human bone marrow samples/NMB22-1022_D1/1022D1 correlation analysis.xlsx")
