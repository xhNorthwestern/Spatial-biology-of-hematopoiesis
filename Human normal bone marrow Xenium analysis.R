...
results_folder = '~/Desktop/Xenium/Giotto analysis/Human samples/HumanNormalBM/2'
...
xenium_folder = '~/Desktop/Xenium/Giotto analysis/Human samples/HumanNormalBM'

# general files (some are supplemental files)
settings_path = paste0(xenium_folder, 'experiment.xenium')
he_img_path = paste0(xenium_folder, 'morphology_focus.ome.tif')
if_img_path = paste0(xenium_folder, 'morphology_mip.ome.tif')

# files (SUBCELLULAR): (tutorial focuses on working with these files)
cell_bound_path = paste0('~/Desktop/Xenium/Giotto analysis/Human samples/HumanNormalBM/cell_boundaries.csv.gz')
nuc_bound_path = paste0('~/Desktop/Xenium/Giotto analysis/Human samples/HumanNormalBM/nucleus_boundaries.csv.gz')
tx_path = paste0('~/Desktop/Xenium/Giotto analysis/Human samples/HumanNormalBM/transcripts.csv.gz')
feat_meta_path = paste0('~/Desktop/Xenium/Giotto analysis/Human samples/HumanNormalBM/cell_feature_matrix/features.tsv.gz') # (also used in aggregate)
...
plot(gpoints_list$`Gene Expression`,  # 77.843 sec elapsed
     feats = c('GYPA', 'C1QB'))
tx_dt_types$`Gene Expression`[feat_ID %in% c('GYPA', 'C1QB'), table(feat_ID)]
...
Xenium_labels<-c("Dendritic cell",#1
                 "Erythroid progenitor cell",#2
                 "Neutrophil_LTF high",#3
                 "Neutrophil_PRTN3 high",#4
                 "Erythroid cell",#5
                 "T cell",#6
                 "Neutrophil_LTF high",#7
                 "Neutrophil_PRTN3 high",#8
                 "C1Q+ Macrophage",#9
                 "Stromal cell",#10
                 "T cell",#11
                 "Erythroid cell",#12
                 "B cell",#13
                 "Neutrophil_LTF high",#14
                 "T cell",#15
                 "Epithelial cell",#16
                 "Erythroid cell")#17
names(Xenium_labels)<-1:17
...
mycolorcode = c('magenta', 'red', 'lavender','cyan', 
                'purple', 'yellowgreen', 'yellow', 'plum', 'grey', 'orange')
names(mycolorcode) = c('Erythroid progenitor cell', 'Erythroid cell','T cell', 'Dendritic cell', 
                       'Stromal cell', 'C1Q+ Macrophage', 'B cell', 'Neutrophil_LTF high', 'Epithelial cell', 'Neutrophil_PRTN3 high')
dimPlot2D(gobject = xenium_gobj,     dim_reduction_name = 'umap',
          cell_color = "Xenium_labels", cell_color_code = mycolorcode, show_NN_network = F, point_size = 1.5, legend_text = 16,
          legend_symbol_size = 3, axis_text = 16, axis_title = 16,
          save_param = list(save_name = "10_d_Annotation"))

dimPlot2D(gobject = xenium_gobj,     dim_reduction_name = 'umap',
          cell_color = "Xenium_labels", cell_color_code = mycolorcode, show_NN_network = F, point_size = 1.5, show_legend = F, legend_text = 16,
          legend_symbol_size = 3, axis_text = 16, axis_title = 16,
...
spatPlot2D(gobject = xenium_gobj,
           spat_unit = 'cell',
           cell_color = 'Xenium_labels',
           point_size = 0.1,
           point_shape = 'no_border',
           background_color = 'black',
           show_legend = TRUE,
           save_param = list(
             save_name = '6_a_spat_leiden',
             base_width = 15,
             base_height = 15))

subloc = subsetGiottoLocs(xenium_gobj,
                          x_min = 3000, x_max = 4000,
                          y_min = 1000, y_max = 2000,
                          poly_info = 'cell')

# show subset of genes
spatInSituPlotPoints(subloc,
                     feats = list('rna' = c("GYPA", "C1QA", 'C1QC')),
                     point_size = 0.6,
                     show_polygon = TRUE,
                     polygon_feat_type = 'cell',
                     polygon_color = 'white',
...
subloc4 = subsetGiottoLocs(xenium_gobj,
                           x_min = 3180, x_max = 3450,
                           y_min = 1280, y_max = 1500,
                           poly_info = 'cell')

# show subset of genes

spatInSituPlotPoints(subloc4,
                     show_image = FALSE,
                     feats = list('rna' = c(
                       "GYPA", "C1QB")),
                     feats_color_code = c(
...
spatInSituPlotPoints(subloc4,
                     ...
                     show_legend = TRUE,
                     save_param = list(
                       save_name = '16_c_subset_in_situ'))

spatPlot2D(gobject = subloc4, point_size = 6.0,
           legend_text = 16,
...
subloc5 = subsetGiottoLocs(xenium_gobj,
                           x_min = 6000, x_max = 7000,
                           y_min = 4500, y_max = 5500,
                           poly_info = 'cell')

# show subset of genes

spatInSituPlotPoints(subloc5,
                     show_image = FALSE,
                     feats = list('rna' = c(
                       "GYPA", "C1QB")),
                     feats_color_code = c(
...
spatInSituPlotPoints(subloc5,
                     ...
                     show_legend = TRUE,
                     save_param = list(
                       save_name = '17_c_subset_in_situ'))

spatPlot2D(gobject = subloc5, point_size = 6.0,
           legend_text = 16,
           legend_symbol_size = 4,
           axis_text = 12,
           axis_title = 12,
...
subloc6 = subsetGiottoLocs(xenium_gobj,
                           x_min = 6400, x_max = 6800,
                           y_min = 5150, y_max = 5500,
                           poly_info = 'cell')

# show subset of genes

spatInSituPlotPoints(subloc6,
                     show_image = FALSE,
                     feats = list('rna' = c(
                       "GYPA", "C1QB")),
                     feats_color_code = c(
...
spatInSituPlotPoints(subloc6,
                     ...
                     show_legend = TRUE,
                     save_param = list(
                       save_name = '18_c_subset_in_situ'))

spatPlot2D(gobject = subloc6, point_size = 6.0,
           legend_text = 16,
           legend_symbol_size = 4,
           axis_text = 12,
           axis_title = 12,
...
subloc7 = subsetGiottoLocs(xenium_gobj,
                           x_min = 6500, x_max = 6600,
                           y_min = 5250, y_max = 5350,
                           poly_info = 'cell')

# show subset of genes

spatInSituPlotPoints(subloc7,
                     show_image = FALSE,
                     feats = list('rna' = c(
                       "GYPA", "C1QB")),
                     feats_color_code = c(
...
spatInSituPlotPoints(subloc7,
                     ...
                     show_legend = TRUE,
                     save_param = list(
                       save_name = '19_c_subset_in_situ'))

spatPlot2D(gobject = subloc7, point_size = 6.0,
           legend_text = 16,
           legend_symbol_size = 4,
           axis_text = 12,
           axis_title = 12,
...
spatPlot2D(gobject = subloc7, point_size = 6.0,
           ...
           axis_title = 12,
           cell_color = 'Xenium_labels', cell_color_code = mycolorcode,
           select_cell_groups = c('C1Q+ Macrophage','Erythroid cell', 'Erythroid progenitor cell'), show_other_cells = F)
...
spatPlot2D(gobject = subloc7, point_size = 6.0,
           ...
           axis_title = 12,
           cell_color = 'Xenium_labels', cell_color_code = mycolorcode,
           select_cell_groups = c('C1Q+ Macrophage','B cell', 'Dendritic cell', 'Neutrophil_LTF high', 'Neutrophil_PRTN3 high', 'Stromal cell', 'T cell'), show_other_cells = F)
...
write_xlsx(xenium_gobj@cell_metadata[["cell"]][["rna"]]@metaDT, "~/Desktop/Xenium/Giotto analysis/Human samples/HumanNormalBM/2/NormalBMcellmeta.xlsx")


write_xlsx(cell_proximities[["enrichm_res"]], "~/Desktop/Xenium/Giotto analysis/Human samples/HumanNormalBM/2/NormalBMcellproximity.xlsx")
