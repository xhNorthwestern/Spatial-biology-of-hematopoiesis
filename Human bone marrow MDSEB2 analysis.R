...
results_folder = '~/Desktop/Xenium/Giotto analysis/Human samples/MDSEB2/2'
...
xenium_folder = '~/Desktop/Xenium/Giotto analysis/Human samples/MDSEB2'
...
cell_bound_path = paste0('~/Desktop/Xenium/Giotto analysis/Human samples/MDSEB2/cell_boundaries.csv.gz')
nuc_bound_path = paste0('~/Desktop/Xenium/Giotto analysis/Human samples/MDSEB2/nucleus_boundaries.csv.gz')
tx_path = paste0('~/Desktop/Xenium/Giotto analysis/Human samples/MDSEB2/transcripts.csv.gz')
feat_meta_path = paste0('~/Desktop/Xenium/Giotto analysis/Human samples/MDSEB2/cell_feature_matrix/features.tsv.gz') # (also used in aggregate)
...
plotTSNE(gobject = xenium_gobj,
         ...
         point_size = 0.01,
         point_shape = 'no_border',
         save_param = list(save_name = '5_tsne_leiden'))
...
Xenium_labels<-c("Neutrophil_PRTN3 high",#1
                 "Neutrophil_LTF high",#2
                 "Neutrophil_LTF high",#3
                 "T cell",#4
                 "Neutrophil_LTF high",#5
                 "Other Macrophage",#6
                 "Erythroid cell",#7
                 "C1Q+ Macrophage",#8
                 "Neutrophil_PRTN3 high",#9
                 "Stromal cell",#10
                 "Erythroid progenitor cell",#11
                 "Stromal cell",#12
                 "B cell (Plasmocyte)",#13
                 "Endothelial cell",#14
                 "Erythroid cell",#15
                 "B cell (Centrocyte)",#16
                 "C1Q+ Macrophage",#17
                 "T cell")#18
names(Xenium_labels)<-1:18
...
mycolorcode = c('magenta', 'red', 'lavender','cyan', 
                'purple', 'yellowgreen', 'yellow', 'plum', 'grey', 'orange', 'lightblue')
names(mycolorcode) = c('Erythroid progenitor cell', 'Erythroid cell','T cell', 'B cell (Plasmocyte)', 
                       'Stromal cell', 'C1Q+ Macrophage', 'B cell (Centrocyte)', 'Neutrophil_LTF high', 'Endothelial cell', 'Neutrophil_PRTN3 high', 'Other Macrophage')
...
spatInSituPlotPoints(subloc1,
                     ...
                     coord_fix_ratio = TRUE,
                     save_param = list(
                       save_name = '14_a_subset_in_situ'))

subloc3 = subsetGiottoLocs(xenium_gobj,
                           x_min = 2320, x_max = 2550,
                           y_min = 1800, y_max = 2000,
                           poly_info = 'cell')

# show subset of genes

spatInSituPlotPoints(subloc3,
                     show_image = FALSE,
                     feats = list('rna' = c(
                       "GYPA", "C1QB")),
                     feats_color_code = c(
...
spatInSituPlotPoints(subloc3,
                     ...
                     show_legend = TRUE,
                     save_param = list(
                       save_name = '16_b_subset_in_situ'))

spatPlot2D(gobject = subloc3, point_size = 6.0,
           legend_text = 16,
...
spatPlot2D(gobject = subloc3, point_size = 6.0,
           ...
           axis_title = 12,
           cell_color = 'Xenium_labels', cell_color_code = mycolorcode,
           select_cell_groups = c('C1Q+ Macrophage', 'T cell', 'B cell (Plasmocyte)', 'Stromal cell', 'B cell (Centrocyte)', 'Neutrophil_LTF high', 'Endothelial cell', 'Neutrophil_PRTN3 high', 'Other Macrophage'), show_other_cells = F)


library(writexl)

write_xlsx(xenium_gobj@cell_metadata[["cell"]][["rna"]]@metaDT, "~/Desktop/Xenium/Giotto analysis/Human samples/MDSEB2/2/MDSEB2cellmeta.xlsx")


write_xlsx(cell_proximities[["enrichm_res"]], "~/Desktop/Xenium/Giotto analysis/Human samples/MDSEB2/2/MDSEB2cellproximity.xlsx")
