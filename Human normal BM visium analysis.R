results_folder = '~/Desktop/Visium/The first four human bone marrow samples/Matrix/NMB21-141C1/2'
...
data_path = '~/Desktop/Visium/The first four human bone marrow samples/Matrix/NMB21-141C1'


visium_YoungControl = createGiottoVisiumObject(visium_dir = data_path,
                                      expr_data = 'raw',
                                      png_name = 'tissue_lowres_image.png',
                                      gene_column_index = 2,
                                      instructions = instrs)

showGiottoImageNames(visium_YoungControl)

pDataDT(visium_YoungControl)

spatPlot2D(gobject = visium_YoungControl, cell_color = 'in_tissue', point_size = 2,
           cell_color_code = c('0' = 'lightgrey', '1' = 'blue'),
           show_image = T, image_name = 'image')

metadata = pDataDT(visium_YoungControl)
in_tissue_barcodes = metadata[in_tissue == 1]$cell_ID
visium_YoungControl = subsetGiotto(visium_YoungControl, cell_ids = in_tissue_barcodes)


visium_YoungControl <- filterGiotto(gobject = visium_YoungControl,
                           expression_threshold = 1,
                           feat_det_in_min_cells = 20,
                           min_det_feats_per_cell = 70,
                           expression_values = c('raw'),
                           verbose = T)

visium_YoungControl <- normalizeGiotto(gobject = visium_YoungControl, scalefactor = 6000, verbose = T)


visium_YoungControl <- addStatistics(gobject = visium_YoungControl)


spatPlot2D(gobject = visium_YoungControl, show_image = T, point_alpha = 0.7,
           cell_color = 'nr_feats', color_as_factor = F)


visium_YoungControl <- calculateHVF(gobject = visium_YoungControl, save_plot = TRUE)

gene_metadata = fDataDT(visium_YoungControl)
featgenes = gene_metadata[hvf == 'yes' & perc_cells > 3 & mean_expr_det > 0.4]$feat_ID


visium_YoungControl <- runPCA(gobject = visium_YoungControl,
                     genes_to_use = featgenes)

screePlot(visium_YoungControl, ncp = 30)

plotPCA(gobject = visium_YoungControl)


visium_YoungControl <- runUMAP(visium_YoungControl, dimensions_to_use = 1:10)
plotUMAP(gobject = visium_YoungControl)

visium_YoungControl <- runtSNE(visium_YoungControl, dimensions_to_use = 1:10, check_duplicates = FALSE)
plotTSNE(gobject = visium_YoungControl)


visium_YoungControl <- createNearestNetwork(gobject = visium_YoungControl, dimensions_to_use = 1:10, k = 15)


visium_YoungControl <- doLeidenCluster(gobject = visium_YoungControl, resolution = 0.2, n_iterations = 1000)

dimPlot2D(gobject = visium_YoungControl,     dim_reduction_name = 'umap',
...
dimPlot2D(gobject = visium_YoungControl,     dim_reduction_name = 'umap',
          cell_color = "leiden_clus", show_NN_network = F, label_size = 10, point_size = 6.0, show_legend = F, legend_text = 20, show_center_label = F,
          legend_symbol_size = 6, axis_text = 16, axis_title = 16,
          save_param = list(save_name = "7_b_Annotation"))


spatDimPlot(gobject = visium_YoungControl, cell_color = 'leiden_clus',
            dim_point_size = 3, spat_point_size = 3.5)

spatDimPlot(gobject = visium_YoungControl, cell_color = 'nr_feats', color_as_factor = F,
            dim_point_size = 3, spat_point_size = 3.5)

spatPlot2D(visium_YoungControl, cell_color = 'leiden_clus', label_size = 10,
           coord_fix_ratio = 1, point_size = 4.0, show_legend = F, axis_text = 16,
           axis_title = 16)

spatPlot2D(visium_YoungControl, cell_color = 'leiden_clus',
...
gini_markers_subclusters = findMarkers_one_vs_all(gobject = visium_YoungControl,
                                                  method = 'gini',
...
violinPlot(visium_YoungControl, feats = unique(topgenes_gini), cluster_column = 'leiden_clus',
           strip_text = 8, strip_position = 'right',
           save_param = list(base_width = 5, base_height = 10))


plotMetaDataHeatmap(visium_YoungControl, selected_feats = unique(topgenes_gini),
                    metadata_cols = c('leiden_clus'),
                    x_text_size = 10, y_text_size = 10)


dimFeatPlot2D(visium_YoungControl, expression_values = 'scaled',
...
scran_markers_subclusters = findMarkers_one_vs_all(gobject = visium_YoungControl,
                                                   method = 'scran',
                                                   expression_values = 'normalized',
                                                   cluster_column = 'leiden_clus')
...
violinPlot(visium_YoungControl, feats = unique(topgenes_scran), cluster_column = 'leiden_clus',
           strip_text = 10, strip_position = 'right',
           save_param = list(base_width = 5))

plotMetaDataHeatmap(visium_YoungControl,  x_text_size = 20,
                    x_text_angle = 45,
                    y_text_size = 20,
                    strip_text_size = 20, selected_feats = topgenes_scran[c(1:3, 5:6, 8:16, 18)],
                    metadata_cols = c('leiden_clus'))
...
visium_YoungControl = runPAGEEnrich(gobject = visium_YoungControl, sign_matrix = PAGE_matrix_1, min_overlap_genes = 2)

cell_types_PAGE = colnames(PAGE_matrix_1)
plotMetaDataCellsHeatmap(gobject = visium_YoungControl,
...
spatCellPlot2D(gobject = visium_YoungControl,
               spat_enr_names = 'PAGE',
               cell_annotation_values = cell_types_PAGE[9:12],
               cow_n_col = 2,coord_fix_ratio = 1, point_size = 3, show_legend = T)


spatDimCellPlot2D(gobject = visium_YoungControl,
                  spat_enr_names = 'PAGE',
...
write_xlsx(Spatialcorrelation@enrichDT, "~/Desktop/Visium/The first four human bone marrow samples/Matrix/NMB21-141C1/2/141C1 correlation analysis.xlsx")
